...
{ 
  ...
  fd = open(MODEMDEVICE, O_RDWR|O_NOCTTY);  //O_RDWR 可讀可寫,O_NOCTTY 如果路徑名指向終端設備,不要把這個設備用作控制終端  
  if (fd < 0)                               //如果不設定這個旗標，有些輸入(例如鍵盤的abort)信號可能影響程式。
  {                             
    perror(MODEMDEVICE);
    exit(1);
  }
  ...
  tcgetattr(fd, &oldtio);    //用來取得目前的串列埠參數值，tcgetattr()取得檔案描述子fd後，將其存入&oldtio
  bzero(&newtio, sizeof(newtio));

  newtio.c_cflag = BAUDRATE|CS8|CLOCAL|CREAD;  //鮑率設定｜字符長度:8bits｜忽略調制解調器線路狀態｜使用接收器
  newtio.c_iflag = IGNPAR;  //忽略奇校正錯誤
  newtio.c_oflag = 0;
  newtio.c_lflag = ICANON;  //使用標準輸入模式

  tcflush(fd, TCIFLUSH);  //清除所有佇列在串列埠的輸入和輸出，其中fd為被處理的串列埠，引數TCIFLUSH為清除輸入 
  tcsetattr(fd, TCSANOW, &newtio);     //用來設定串列埠參數值，tcsetattr()執行後使用newtio指向的termios資料結構，重新設定檔案描述子fd，其中引數TCSANOW是立即將值改變
